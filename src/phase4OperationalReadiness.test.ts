...
const todayPilotSanitizedSummaryDocPath = resolve(
  process.cwd(),
  'docs/TODAY_REAL_FEED_PILOT_SANITIZED_SUMMARY.md',
);
...
test('phase 4 manual readiness assets exist for manual SQL rollout', () => {
  ...
  assert.equal(existsSync(todayPilotSanitizedSummaryDocPath), true);
...
test('README records the Today pilot workflow through Task 45 without switching Today by default', () => {
  const readme = readFileSync(readmePath, 'utf8');

  assert.match(readme, /Tasks 14A-45/i);
  ...
  assert.match(readme, /phase4:update-today-evidence/i);
  assert.match(readme, /phase4:today-evidence-next/i);
  assert.match(readme, /phase4:today-evidence-status/i);
  assert.match(readme, /phase4:today-pilot-report/i);
  assert.match(readme, /phase4:today-help/i);
  assert.match(readme, /--preset/i);
  ...
  assert.match(readme, /Do not commit local\/private report files/i);
  assert.match(readme, /Keep updating one local\/private evidence file across multiple pilot sessions/i);
  assert.match(readme, /current evidence result still remains `continue_pilot`/i);
...
test('Today real-feed rollout decision doc captures Task 20 blockers, enablement criteria, and rollback steps without switching defaults', () => {
  ...
  assert.match(doc, /maintained through Task 45/i);
  ...
  assert.match(doc, /phase4:update-today-evidence/i);
  assert.match(doc, /phase4:today-evidence-next/i);
  assert.match(doc, /phase4:today-pilot-report/i);
  assert.match(doc, /phase4:today-help/i);
  assert.match(doc, /separate controlled rollout task/i);
});
...
test('Today real-feed target pilot doc captures pilot-only env, QA, rollback, and stop criteria without changing defaults', () => {
  ...
  assert.match(doc, /Do not commit local\/private evidence or local\/private report files/i);
});

test('Today pilot docs mention the local helper, evidence starter, update, report, and review commands while staying bounded to local QA only', () => {
  const readme = readFileSync(readmePath, 'utf8');
  const manualQaDoc = readFileSync(manualQaDocPath, 'utf8');
  ...
  assert.match(manualQaDoc, /phase4:update-today-evidence/i);
  assert.match(manualQaDoc, /phase4:today-evidence-review/i);
  assert.match(manualQaDoc, /phase4:today-evidence-next/i);
  assert.match(manualQaDoc, /phase4:today-pilot-report/i);
  assert.match(manualQaDoc, /phase4:today-help/i);
  assert.match(manualQaDoc, /local\/private evidence files should not be committed/i);
  assert.match(manualQaDoc, /local\/private report files/i);
  assert.match(handoffDoc, /phase4:today-pilot-check/i);
  assert.match(handoffDoc, /phase4:create-today-evidence/i);
  assert.match(handoffDoc, /phase4:update-today-evidence/i);
  assert.match(handoffDoc, /phase4:today-evidence-review/i);
  assert.match(handoffDoc, /phase4:today-evidence-next/i);
  ...
  assert.match(handoffDoc, /Phase 4 Task 46/i);
...
test('Today real-feed pilot evidence doc records objective env rollback evidence and default-switch gates', () => {
  ...
  assert.match(doc, /docs\/evidence\/today-real-feed-pilot-report\.local\.md/i);
  assert.match(doc, /gitignored `docs\/evidence\/\*\.local\.\*` or `docs\/evidence\/\*\.private\.\*` paths/i);
  assert.match(doc, /docs\/examples\/today-real-feed-pilot-evidence\*\.json/i);
  assert.match(doc, /local env keys are present/i);
  assert.match(doc, /bilingual/i);
  assert.match(doc, /mobile/i);
  assert.match(doc, /freshnessNotes/i);
  assert.match(doc, /sourceCoverageNotes/i);
  assert.match(doc, /RLS/i);
  assert.match(doc, /no frontend writes/i);
  assert.match(doc, /no frontend AI calls/i);
  assert.match(doc, /should not be committed/i);
  assert.match(doc, /separate controlled rollout task is still required/i);
});
...
test('pilot operator checklist doc exists and mentions rollback plus screenshot privacy boundaries', () => {
  ...
  assert.match(doc, /1\.\s+Baseline mock confirmation/i);
  assert.match(doc, /2\.\s+Real-feed env confirmation/i);
  assert.match(doc, /3\.\s+Today real-card render check/i);
  assert.match(doc, /4\.\s+Detail page safety check/i);
  assert.match(doc, /5\.\s+Source\/provenance check/i);
  assert.match(doc, /6\.\s+Enrichment behavior check/i);
  assert.match(doc, /7\.\s+Filter behavior check/i);
  assert.match(doc, /8\.\s+real_empty check/i);
  assert.match(doc, /9\.\s+Mobile quality check/i);
  assert.match(doc, /10\.\s+Freshness check/i);
  assert.match(doc, /11\.\s+Source coverage check/i);
  assert.match(doc, /12\.\s+Rollback-to-mock check/i);
  assert.match(doc, /13\.\s+Evidence update commands/i);
  assert.match(doc, /14\.\s+Evidence review command/i);
  assert.match(doc, /15\.\s+Sanitized report command/i);
  assert.match(doc, /phase4:today-pilot-check/i);
  assert.match(doc, /phase4:create-today-evidence/i);
  assert.match(doc, /phase4:update-today-evidence/i);
  assert.match(doc, /phase4:today-evidence-status/i);
  ...
  assert.match(doc, /phase4:today-help/i);
  assert.match(doc, /gitignored `docs\/evidence\/\*\.local\.\*` or `docs\/evidence\/\*\.private\.\*` paths/i);
  assert.match(doc, /preview-read policy failures or wrong-project wiring/i);
  ...
  assert.match(doc, /Do not commit your local\/private pilot report files either/i);
  assert.match(doc, /Exact Field Mapping For The Current Missing Evidence/i);
  assert.match(doc, /what to do/i);
  assert.match(doc, /what to observe/i);
  assert.match(doc, /what evidence field to update/i);
  assert.match(doc, /what screenshot\/note to collect/i);
  assert.match(doc, /what counts as pass/i);
  assert.match(doc, /what counts as fail/i);
...
test('.gitignore protects local and private operator evidence files', () => {
  ...
  assert.match(gitignore, /docs\/evidence\/\*\.local\.md/i);
  assert.match(gitignore, /docs\/evidence\/\*\.private\.md/i);
});

test('sanitized Today pilot summary exists and keeps Today mock-by-default while recording incomplete evidence honestly', () => {
  const doc = readFileSync(todayPilotSanitizedSummaryDocPath, 'utf8');

  assert.match(doc, /sanitized summary/i);
  assert.match(doc, /Today remains mock-by-default/i);
  assert.match(doc, /continue_pilot/i);
  assert.match(doc, /real_empty/i);
  assert.match(doc, /completed and non-empty enriched content/i);
  assert.match(doc, /No frontend AI calls were added/i);
  assert.match(doc, /No frontend content writes were added/i);
  assert.match(doc, /No scheduled AI enrichment was added/i);
  assert.match(doc, /phase4:today-evidence-next/i);
});
...
test('controlled default rollout plan stays planning-only and preserves rollout boundaries', () => {
  ...
  assert.match(doc, /planning-only/i);
  assert.match(doc, /maintained through Phase 4 Task 45/i);
  assert.match(doc, /Today remains mock by default/i);
  assert.match(doc, /VITE_USE_REAL_CONTENT_FEED=false/i);
  assert.match(doc, /VITE_USE_REAL_CONTENT_FEED=true/i);
  assert.match(doc, /rollback/i);
  ...
  assert.match(doc, /Tasks 25 through 45/i);
});
